      ...
      <div style={{ background: '#0D1017', borderRadius: '12px', border: '1px solid #3B82F630', padding: '20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div>
            ...
            <input
              value={access.email}
              onChange={e => setAccess(a => ({ ...a, email: e.target.value }))}
              placeholder="jean.dupont@viesionnaire.fr"
              type="email"
              style={inputStyle}
            />
          </div>
          ...
          {pwdErrors.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
              {pwdErrors.map(e => (
                <div key={e} style={{ fontSize: '11px', color: '#EF4444' }}>✕ {e}</div>
              ))}
            </div>
          )}
          <div>
            ...
            <select
              value={access.role}
              onChange={e => setAccess(a => ({ ...a, role: e.target.value }))}
              style={selectStyle}
            >
          ...
          {accessStatus && (
            <div style={{
              background: accessStatus.ok ? '#22C55E15' : '#EF444415',
              border: `1px solid ${accessStatus.ok ? '#22C55E30' : '#EF444430'}`,
              borderRadius: '8px', padding: '10px 14px',
              fontSize: '13px', color: accessStatus.ok ? '#22C55E' : '#EF4444',
            }}>
              {accessStatus.msg}
            </div>
          )}
          <button
            ...
            disabled={submitting}
            style={{
              background: submitting ? '#1E2535' : 'linear-gradient(135deg, #3B82F6, #8B5CF6)',
