...
type Vehicle = {
  ...
  type: string;
  status: string;
  metadata: { diplomeRequis?: string; equipements?: string } | null;
};
    ...
    <div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #1E2535' }}>
              {['N° Véhicule', 'Type', 'Diplôme requis', 'Équipements', 'Statut', 'Actions'].map(h => (
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} style={{ padding: '24px', textAlign: 'center', color: '#6B7A99', fontSize: '13px' }}>Aucun véhicule enregistré</td></tr>
            ) : vehicles.map(v => (
      ...
      <div style={{ background: '#0D1017', borderRadius: '12px', border: '1px solid #2A3348', padding: '16px', display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <button
          ...
          style={{ background: adding ? '#1E2535' : '#14B8A6', color: adding ? '#6B7A99' : 'white', border: 'none', borderRadius: '8px', padding: '8px 16px', cursor: adding ? 'not-allowed' : 'pointer', fontWeight: '600', fontSize: '13px', whiteSpace: 'nowrap' }}
        >{adding ? 'Ajout...' : '+ Ajouter'}</button>
      </div>
      ...
      {qrVehicle && (
        ...
        >
          <div
            ...
            style={{ background: '#0D1017', border: '1px solid #1E2535', borderRadius: '16px', padding: '32px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px', minWidth: '280px' }}
          >
            ...
            <div style={{ background: 'white', padding: '16px', borderRadius: '12px' }}>
              <QRCodeCanvas
                id="qr-dl-canvas"
                value={`https://holdingbsc.re/pda/scan?vehicle=${qrVehicle.plate}`}
                size={180}
                level="H"
              />
            </div>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <button
                onClick={downloadQr}
                style={{ background: 'linear-gradient(135deg, #14B8A6, #3B82F6)', color: 'white', border: 'none', borderRadius: '8px', padding: '10px 20px', cursor: 'pointer', fontWeight: '600', fontSize: '13px' }}
              >⬇ Télécharger PNG</button>
              <button
                onClick={() => setQrVehicle(null)}
                style={{ background: 'transparent', color: '#6B7A99', border: '1px solid #2A3348', borderRadius: '8px', padding: '10px 20px', cursor: 'pointer', fontSize: '13px' }}
              >Fermer</button>
            </div>
          </div>
        </div>
